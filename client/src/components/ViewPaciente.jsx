  ...
  return (
    <div className='row col-10 mx-auto h-full'>
      <section className='col-7 p-4 h-full'>
        ...
        <div
          id='documentos'
          className='row mx-auto bg-secondary-subtle rounded p-5 mt-5 h-full'
        >
      ...
      <section className='col-5 p-4'>
        <div
          className='row mx-auto bg-secondary-subtle rounded p-5 overflow-auto'
          style={{ height: "80%" }}
        >
          <h1 className='display-1 fs-1 text-center'>Fichas Paciente</h1>
          {fichas.length === 0 ? (
            <p className='text-secondary display-1 fs-1 text-center'>
              El paciente no cuenta con fichas registradas
            </p>
          ) : (
            fichas.map((ficha) => <FichaPreview key={ficha.id} data={ficha} />)
          )}
        </div>
      </section>
    </div>
